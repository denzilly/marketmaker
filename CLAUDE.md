...
- **P&L**: Mark-to-market against the `positions` view's `mark_price` (unitless, not currency-specific). That's `assets.last_price` — the last *book* trade — falling back to the most recent trade of any kind, so an asset that has only ever traded OTC still marks instead of showing no P&L. Settled assets mark against `settlement_value` as before.
